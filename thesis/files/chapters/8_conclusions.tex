...
La tabella~\ref{tab:copertura-requisiti} mostra la copertura dei requisiti elencati nella Sezione~\ref{sec:elenco-requisiti} (Tabella~\ref{tab:requisiti}): per ciascun requisito è indicato se è stato soddisfatto o meno.
...
\addcontentsline{lot}{table}{\thetable~~~Tabella di copertura dei requisiti}
\vspace{0.5cm}
\tablefirsthead{
  \hiderowcolors
  \hline
  \textbf{Identificativo} & \textbf{Soddisfatto} \\
  \hline
  \showrowcolors
}
\tablehead{
...
\begin{supertabular}{|p{2.75cm}|p{2.75cm}|}
\hline
R.O.0 & Sì \\ \hline
R.O.1 & Sì \\ \hline
R.O.2 & Sì \\ \hline
...
\begin{center}
\textbf{Tabella \thetable:} Tracciamento del soddisfacimento dei requisiti
\label{tab:copertura-requisiti}
\end{center}


\section{Analisi del prodotto}
Il prodotto sviluppato è già in uso presso il cliente e ora sta passando una fase di monitoraggio e raccolta \textit{feedback} per futuri miglioramenti. Di seguito sono riportati i risultati ottenuti fino alla data di fine stage.
\subsection{Previsioni annuali}
Le previsioni hanno raggiunto un grado soddisfacente sull’andamento generale annuale: il \acrshort{wape} medio è rimasto sotto il 15\%, con una buona coerenza tra stagionalità attese e valori stimati. La dashboard di monitoraggio mostra l’allineamento tra curva storica e serie prevista, evidenziando scarti contenuti nei mesi di picco.
La Figura~\ref{fig:forecast-annual} illustra l’andamento storico a confronto con le previsioni annuali, mentre la Figura~\ref{fig:forecast-annual-table} riporta il dettaglio numerico delle stime per ciascun periodo. La tabella mette affiancati valore storico, previsione e scostamento percentuale mese per mese, così da evidenziare dove il modello sovrastima o sottostima.
\begin{figure}[H]
    \centering
...
\end{itemize}
La Figura~\ref{fig:feature-importance} visualizza la distribuzione dei contributi SHAP per le feature principali e aiuta a interpretare il segno e l’intensità dell’impatto sulle previsioni.
\begin{figure}[H]
    \centering
...
\begin{itemize}
\item \textbf{Previsioni per singoli articoli}: per quanto l'andamento generale sia buono, le previsioni a livello di singolo articolo mostrano scostamenti più marcati soprattutto per articoli con bassa storicità o stagionalità irregolare. Si prevede di integrare modelli specifici per cluster di articoli simili.
